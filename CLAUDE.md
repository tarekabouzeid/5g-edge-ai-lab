...
Open5GS core → real minikube edge cluster → real GPU video/VLM inference —
...
./kubeconform -strict -summary edge/manifests/*.yaml monitoring/manifests/*.yaml
...
./lab.sh <core|ran|minikube|edge-apps|breakout|monitoring|portal> <up|down|...>
./lab.sh status
...
`edge/setup-minikube-breakout.sh`, `monitoring/deploy.sh`,
`scripts/fetch-demo-media.sh`, `scripts/demo-stream-from-ue.sh`,
`scripts/stream-test-video.sh`, `scripts/host-gpu-exporter.py`,
`ran/rls-watchdog.sh`.

The edge cluster is **minikube** (the only cluster path; K3s + GPU Operator
and a KIND dev path were removed on 2026-09-25 — they're in git history if
ever needed). On WSL2 + an NVIDIA GPU the whole lab comes up
...
- **No interactive `sudo`.** This session's shell has no TTY to answer a
  ...
  Toolkit install, `modprobe sctp` on non-WSL2 hosts) has to be
...
- **WSL2 specifically has its own gotchas beyond stock Ubuntu** — the
  ...
  Windows-side driver and `nvidia-smi` already work (README "Running on
  WSL2"), vLLM hangs under WSL2's GPU passthrough (hence llama.cpp,
  `docs/phase-notes/phase-6.md`), and NVIDIA's DCGM doesn't run there
  (hence `scripts/host-gpu-exporter.py`). Check `docs/phase-notes/phase-4.md`'s
  "Known risks" before re-diagnosing from scratch.
...
| `edge/` | 3–6 | minikube + GPU bring-up (`minikube-up.sh`) and its no-sudo breakout (`setup-minikube-breakout.sh`); the ingestion image (`edge/ingest/`: RTSP → YOLOv8n on CPU → VLM, plus the API the portal uses) and the gateway/ingest/VLM manifests (VLM = llama.cpp + Gemma 4 E4B, the only GPU consumer) |
| `monitoring/` | 8 | Prometheus + Grafana on minikube; GPU panels fed by the host-side `scripts/host-gpu-exporter.py` (nvidia-smi → `DCGM_FI_DEV_*` metric names) |
...
address all the way to the edge cluster's gateway — this is the local-breakout
...
the GPU/CUDA `edge/ingest` image is `workflow_dispatch`-only (manual) — that image needs a real GPU to mean
