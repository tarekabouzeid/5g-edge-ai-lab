...
| minikube single node (K3s as the bare-metal alternative) | Cell-site or aggregation-site edge Kubernetes cluster | Production would run a hardened, HA control plane and likely a CNI/service mesh choice driven by the operator's existing platform, not a lab-friendly single node |
| NVIDIA device plugin (minikube) / GPU Operator (K3s) | Same tooling, same GPU exposure model | Directly transferable — `nvidia.com/gpu` scheduling is the same mechanism production edge K8s clusters use |
...
| llama.cpp + Qwen2-VL-2B (GGUF) | Metropolis VSS-style RT-VLM microservice | Same serving pattern (OpenAI-compatible API in front of an open VLM); this lab deliberately picked a small (~2B) model for low VRAM/fast iteration over caption accuracy — production would use a larger GPU (e.g. RTX PRO 6000) and a larger/fine-tuned model |
